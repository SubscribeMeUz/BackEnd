...
def login_user(request: sc.UserLogin, db: Session):
    ...
    if isinstance(request, sc.UserLogin):
        if (user is None) or (not auth_handler.verify_password(request.password, user.password)):
            ...
            raise HTTPException(status_code=400, detail="Parol yoki Login noto'g'ri yozilgan!")
...
def create_user(request: sc.UserCreate, db: Session):
    ...
    if user is not None:
        ...
        raise HTTPException(status_code=400, detail={'title': "error",
    ...
    if user is not None:
        ...
        raise HTTPException(status_code=400, detail={"title": "error",
                                                     "error_message": "Phone number is taken"})
    ...
    return {"result": "OK",
            "login": login_user(user_login, db=db)}
