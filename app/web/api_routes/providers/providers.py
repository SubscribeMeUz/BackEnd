import logging
from fastapi import APIRouter, Form, UploadFile, File, Response, Query
...
def get_providers_by_owner(owner_id: int, user=auth()):
...
def delete_provider(provider_id: int, user=admin_auth()):
    return sv.delete_provider(provider_id=provider_id)


@router.post('/add-photo/{provider_id}', status_code=200)
def post_api(provider_id: int, file: UploadFile, user=admin_auth()):
...
def delete_api(photo_id: int, user=admin_auth()):
