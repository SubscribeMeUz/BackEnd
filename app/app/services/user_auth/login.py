...
def verify_otp(phone: str, code: str) -> bool:
    ...
    return sc.OTPPhoneVerifyModel(**{k.decode(): v.decode() for k, v in raw_data.items()})
    try:
...
def send_sms(phone: str, code: str):
    ...
    logger.info(f'OTP response: {resp.json()}')
    return {
        'result': "Ok",
        'message': "OTP sent"
    }
...
def post_request_status(json_body):
    ...
    resp = requests.post(config.TELEGRAM_OTP_URL, headers=HEADERS, json=json_body)
    if resp.ok:
        return resp.json()
    else:
